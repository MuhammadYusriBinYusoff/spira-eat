...
const SingleCategoryList = ({name, categoryMeals}) => {
    return (
        <div className='section-wrapper bg-whitesmoke'>
            <div className='container'>
              <div className='sc-title'>{name}</div>
              <section className='sc-category grid'>
                {
                  categoryMeals.map(categoryMeal => {
                    const { idMeal: id, strMeal: title, strMealThumb: thumbnail} = categoryMeal; //means idCategory will asign to new variable id
                    ...
                    return (
                      <Link to = {`/meal/category/${title}`} className = "category-itm align-center justify-center" key = {id}>
                        <div className='category-itm-img h-100 w-100 flex align-center justify-center'>
                          <img src = {thumbnail} alt = {title} style={{ height: '160px'}}/>
